const { parseProgram, parseExpression } = require("./index");
const a = require("./ast");
...
test('logical precedence', () => {
    ...
    expect(r.kind).toBe('ok');
    expect(r.unsafeGet()).toEqual([
        a.let_('x', a.operator('||', a.bool(true), a.operator('&&', a.bool(false), a.bool(true))))
    ]);
});
...
test('Function definition', () => {
    let r = parseProgram('function foo(a, b) { return a + b; }');
    expect(r.kind).toBe('ok');

    // Handwrite AST using `a` (defined in ast.js)
});

test('Closure variable', () => {
    let r = parseProgram('function foo(a, b) { let closure = function() { return a + b }; return closure; }');
    expect(r.kind).toBe('ok');
...
test('Closure in-line', () => {
    let r = parseProgram('function foo(a, b) { return function() { return a + b } }');
    expect(r.kind).toBe('ok');
...
test('Function call', () => {
    let r = parseProgram('function foo(a, b) { return a + b; } print(foo(1, 2));');
    expect(r.kind).toBe('ok');
